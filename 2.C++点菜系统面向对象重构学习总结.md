...
这是一个重要的理解转折点。我最初的误解是"逻辑相同才要虚函数"，实际上恰恰相反。
...
DevC++ 5.11是这样的，用的ANSI，网上看到的在编译器选项中加入啥代码那个方法也没用。 真以后要在其他IDE上正常显示的话，得记事本那里另存为改编码方式。
...
我自己写的时候记岔了，忘记有`addControl`了，采用了手动管理数组的方式。
