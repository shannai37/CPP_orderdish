...
void CRegisterWin::handle()
{
    ...
    {
        // 1. 获取输入
        char* account = this->arr[2]->getContext();     // 账号
        char* password = this->arr[4]->getContext();    // 密码
        char* repassword = this->arr[6]->getContext();  // 重复密码
        char* roleStr = this->arr[8]->getContext();     // 角色
        ...
        if(userMgr->checkAccount(account))
        {
            system("cls");
            printf("\n\n 账号已存在!\n\n");
            Sleep(1500);
        ...
        // 6. 注册成功
        userMgr->registerUser(account, password, role);
        ...
        this->userRole = role;   // 保存角色
        this->result = 1;        // 1 = 注册成功
    }
    else if(this->flag == -1)  // ESC
    {
        this->result = 0;        // 0 = 返回初始窗口
    }
