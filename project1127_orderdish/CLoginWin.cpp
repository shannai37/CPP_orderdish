...
// 获取用户角色 
// 切换成状态机切换窗口后，我们不会在每个窗口的handle()内做验证身份逻辑
//  
int CLoginWin::getUserRole()
{
...
void CLoginWin::handle()
{
    ...
    {
        // 1. 获取用户名和密码
        char* username = this->arr[1]->getContext();
        char* password = this->arr[3]->getContext();
        ...
        this->result = 1;         // 设置结果：登录成功
    }
    else if(this->flag == 5 || this->flag == -1)  // 取消按钮或ESC
    {
        this->result = 0;    // 设置结果：返回
    }
